...
/**
 * Minimum PHP version; can't call phpmyadmin_fatalError() which uses a
 ...
 */
$GLOBALS['error_handler'] = new phpmyadmin_Error_Handler();
$cfg['Error_Handler']['display'] = true;

/**
 * This setting was removed in PHP 5.4. But at this point phpmyadmin_PHP_INT_VERSION
...
if (! function_exists('mb_detect_encoding')) {
    phpmyadmin_warnMissingExtension('mbstring', $fatal = true);
}

/**
 * the phpmyadmin_Theme class
 */
require './libraries/Theme.class.php';

/**
 * the phpmyadmin_Theme_Manager class
 */
require './libraries/Theme_Manager.class.php';

/**
 * the phpmyadmin_Config class
 */
...
/**
 * the phpmyadmin_Tracker class
 */
require './libraries/Tracker.class.php';

/**
 * the phpmyadmin_Table class
 */
require './libraries/Table.class.php';

/**
 * the phpmyadmin_Types class
 */
require './libraries/Types.class.php';

if (! defined('phpmyadmin_MINIMUM_COMMON')) {
    /**
     * common functions
     */
 ...
 */
$phpmyadmin_PHP_SELF = phpmyadmin_getenv('PHP_SELF');
$_PATH_INFO = phpmyadmin_getenv('PATH_INFO');
if (! empty($_PATH_INFO) && ! empty($phpmyadmin_PHP_SELF)) {
    $path_info_pos = strrpos($phpmyadmin_PHP_SELF, $_PATH_INFO);
    if ($path_info_pos + strlen($_PATH_INFO) === strlen($phpmyadmin_PHP_SELF)) {
        $phpmyadmin_PHP_SELF = substr($phpmyadmin_PHP_SELF, 0, $path_info_pos);
    }
}
$phpmyadmin_PHP_SELF = htmlspecialchars($phpmyadmin_PHP_SELF);
...
if (isset($_POST['usesubform'])) {
    ...
    if (isset($_POST['redirect'])
        && $_POST['redirect'] != basename($phpmyadmin_PHP_SELF)
    ) {
...
if (version_compare(phpversion(), '5.4', 'lt')) {
    ...
    if (function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc()) {
        phpmyadmin_arrayWalkRecursive($_GET, 'stripslashes', true);
        phpmyadmin_arrayWalkRecursive($_POST, 'stripslashes', true);
        phpmyadmin_arrayWalkRecursive($_COOKIE, 'stripslashes', true);
        phpmyadmin_arrayWalkRecursive($_REQUEST, 'stripslashes', true);
    }
}
...
if (! function_exists('preg_replace')) {
    phpmyadmin_warnMissingExtension('pcre', true);
}
...
if (! function_exists('json_encode')) {
    phpmyadmin_warnMissingExtension('json', true);
}

/**
 * @global phpmyadmin_Config $GLOBALS['phpmyadmin_Config']
 * force reading of config file, because we removed sensitive values
 * in the previous iteration
 */
$GLOBALS['phpmyadmin_Config'] = new phpmyadmin_Config(CONFIG_FILE);

if (!defined('phpmyadmin_MINIMUM_COMMON')) {
    $GLOBALS['phpmyadmin_Config']->checkphpmyadminAbsoluteUri();
}
 ...
 */
$GLOBALS['phpmyadmin_Config']->enableBc();

/**
 * clean cookies on upgrade
 * when changing something related to phpmyadmin cookies, increment the cookie version
 */
$phpmyadmin_cookie_version = 4;
if (isset($_COOKIE)
    && (isset($_COOKIE['phpmyadminCookieVer'])
    && $_COOKIE['phpmyadminCookieVer'] < $phpmyadmin_cookie_version)
) {
    // delete all cookies
    foreach ($_COOKIE as $cookie_name => $tmp) {
        $GLOBALS['phpmyadmin_Config']->removeCookie($cookie_name);
    }
    $_COOKIE = array();
    $GLOBALS['phpmyadmin_Config']->setCookie('phpmyadminCookieVer', $phpmyadmin_cookie_version);
}
 ...
 */
if ($GLOBALS['phpmyadmin_Config']->get('ForceSSL')
    && ! $GLOBALS['phpmyadmin_Config']->get('is_https')
) {
    // grab SSL URL
    $url = $GLOBALS['phpmyadmin_Config']->getSSLUri();
    // Actually redirect
    phpmyadmin_sendHeaderLocation($url . phpmyadmin_URL_getCommon($_GET, 'text'));
    // delete the current session, otherwise we get problems (see bug #2397877)
    $GLOBALS['phpmyadmin_Config']->removeCookie($GLOBALS['session_name']);
    exit;
}
 ...
 */
if (! phpmyadmin_checkPageValidity($__redirect, $goto_whitelist)) {
    $__redirect = null;
}
...
if (phpmyadmin_checkPageValidity($_REQUEST['goto'], $goto_whitelist)) {
 ...
 */
if (phpmyadmin_checkPageValidity($_REQUEST['back'], $goto_whitelist)) {
 ...
 * f.e. phpmyadmin_Config: fontsize
 *
 * @todo variables should be handled by their respective owners (objects)
 * f.e. lang, server, collation_connection in phpmyadmin_Config
 */
$token_mismatch = true;
if (phpmyadmin_isValid($_REQUEST['token'])) {
    $token_mismatch = ($_SESSION[' phpmyadmin_token '] != $_REQUEST['token']);
}
...
if ($token_mismatch) {
    ...
    $allow_list = array(
        ...
        'phpMyAdmin',
        /* Cookie preferences */
        'phpmyadmin_lang', 'phpmyadmin_collation_connection',
        /* Possible login form */
        'phpmyadmin_servername', 'phpmyadmin_username', 'phpmyadmin_password',
     ...
     */
    if (defined('phpmyadmin_TEST_THEME')) {
     ...
     * Do actual cleanup
     */
    phpmyadmin_removeRequestVars($allow_list);
 ...
 */
$GLOBALS['db'] = '';
if (phpmyadmin_isValid($_REQUEST['db'])) {
 ...
 */
$GLOBALS['table'] = '';
if (phpmyadmin_isValid($_REQUEST['table'])) {
    // can we strip tags from this?
 ...
 */
if (phpmyadmin_isValid($_REQUEST['selected_recent_table'])) {
 ...
 */
$GLOBALS['sql_query'] = '';
if (phpmyadmin_isValid($_REQUEST['sql_query'])) {
 ...
 */
$GLOBALS['phpmyadmin_Config']->checkPermissions();

if ($GLOBALS['phpmyadmin_Config']->error_config_file) {
...
if ($GLOBALS['phpmyadmin_Config']->error_config_default_file) {
    $error = sprintf(
        __('Could not load default configuration from: %1$s'),
        $GLOBALS['phpmyadmin_Config']->default_source
    );
    trigger_error($error, E_USER_ERROR);
}
if ($GLOBALS['phpmyadmin_Config']->error_phpmyadmin_uri) {
    trigger_error(
        __('The [code]$cfg[\'phpmyadminAbsoluteUri\'][/code] directive MUST be set in your configuration file!'),
        E_USER_ERROR
    );
}
 ...
 * $default_server comes from phpmyadmin_Config::enableBc()
 * @todo merge into phpmyadmin_Config
 */
...
/**
 * @global phpmyadmin_Theme_Manager $_SESSION['phpmyadmin_Theme_Manager']
 */
if (! isset($_SESSION['phpmyadmin_Theme_Manager'])) {
    $_SESSION['phpmyadmin_Theme_Manager'] = new phpmyadmin_Theme_Manager;
} else {
    /**
     * @todo move all __wakeup() functionality into session.inc.php
     */
    $_SESSION['phpmyadmin_Theme_Manager']->checkConfig();
}

// for the theme per server feature
if (isset($_REQUEST['server']) && ! isset($_REQUEST['set_theme'])) {
    $GLOBALS['server'] = $_REQUEST['server'];
    $tmp = $_SESSION['phpmyadmin_Theme_Manager']->getThemeCookie();
    if (empty($tmp)) {
        $tmp = $_SESSION['phpmyadmin_Theme_Manager']->theme_default;
    }
    $_SESSION['phpmyadmin_Theme_Manager']->setActiveTheme($tmp);
    unset($tmp);
}
/**
 * @todo move into phpmyadmin_Theme_Manager::__wakeup()
 */
if (isset($_REQUEST['set_theme'])) {
    // if user selected a theme
    $_SESSION['phpmyadmin_Theme_Manager']->setActiveTheme($_REQUEST['set_theme']);
}

/**
 * the theme object
 * @global phpmyadmin_Theme $_SESSION['phpmyadmin_Theme']
 */
$_SESSION['phpmyadmin_Theme'] = $_SESSION['phpmyadmin_Theme_Manager']->theme;
 ...
 */
$GLOBALS['theme']           = $_SESSION['phpmyadmin_Theme']->getName();
/**
 * the theme path
 * @global string $GLOBALS['phpmyadminThemePath']
 */
$GLOBALS['phpmyadminThemePath']    = $_SESSION['phpmyadmin_Theme']->getPath();
/**
 * the theme image path
 * @global string $GLOBALS['phpmyadminThemeImage']
 */
$GLOBALS['phpmyadminThemeImage']   = $_SESSION['phpmyadmin_Theme']->getImgPath();

/**
 * load layout file if exists
 */
if (@file_exists($_SESSION['phpmyadmin_Theme']->getLayoutFile())) {
    include $_SESSION['phpmyadmin_Theme']->getLayoutFile();
}
...
if (! defined('phpmyadmin_MINIMUM_COMMON')) {
    ...
    if (! empty($_REQUEST['server'])
        ...
        foreach ($cfg['Servers'] as $i => $server) {
            if ($server['host'] == $_REQUEST['server']
                ...
                || $phpmyadmin_String->strtolower($server['verbose']) == $phpmyadmin_String->strtolower($_REQUEST['server'])
                || md5($phpmyadmin_String->strtolower($server['verbose'])) == $phpmyadmin_String->strtolower($_REQUEST['server'])
            ) {
                $_REQUEST['server'] = $i;
                break;
            }
     ...
     * @todo should be done in phpmyadmin_Config
     */
    $GLOBALS['phpmyadmin_Config']->setCookie('phpmyadmin_lang', $GLOBALS['lang']);
    if (isset($GLOBALS['collation_connection'])) {
        $GLOBALS['phpmyadmin_Config']->setCookie(
            'phpmyadmin_collation_connection',
            $GLOBALS['collation_connection']
        );
    }

    $_SESSION['phpmyadmin_Theme_Manager']->setThemeCookie();
    ...
    if (! empty($cfg['Server'])) {
        ...
        // need to be kept for processing in phpmyadmin_Config::loadUserPreferences()
        ...
        if (isset($_SESSION['cache'][$cache_key]['userprefs']['LoginCookieValidity'])) {
            ...
            $GLOBALS['phpmyadmin_Config']->set('LoginCookieValidity', $value);
        ...
        if (isset($cfg['Server']['AllowDeny'])
            ...
            if ($cfg['Server']['AllowDeny']['order'] == 'allow,deny') {
                ...
                if (phpmyadmin_allowDeny('allow')) {
                    $allowDeny_forbidden = false;
                }
                if (phpmyadmin_allowDeny('deny')) {
                    $allowDeny_forbidden = true;
                }
                ...
                if (phpmyadmin_allowDeny('allow') && ! phpmyadmin_allowDeny('deny')) {
                    $allowDeny_forbidden = false;
                } else {
                    $allowDeny_forbidden = true;
                }
            ...
            if ($allowDeny_forbidden) {
                phpmyadmin_logUser($cfg['Server']['user'], 'allow-denied');
        ...
        if (! $cfg['Server']['AllowRoot'] && $cfg['Server']['user'] == 'root') {
            $allowDeny_forbidden = true;
            phpmyadmin_logUser($cfg['Server']['user'], 'root-denied');
            $auth_plugin->authFails();
        }
        ...
        if (! $cfg['Server']['AllowNoPassword']
            ...
            phpmyadmin_logUser($cfg['Server']['user'], 'empty-denied');
            $auth_plugin->authFails();
        }
        ...
        /* Log success */
        phpmyadmin_logUser($cfg['Server']['user']);
         ...
         *  - > 5.0.15
         */
        if (phpmyadmin_MYSQL_INT_VERSION < 50015) {
            phpmyadmin_fatalError(
         ...
         */
        if (phpmyadmin_DRIZZLE) {
            $GLOBALS['phpmyadmin_Types'] = new phpmyadmin_Types_Drizzle();
        } else {
            $GLOBALS['phpmyadmin_Types'] = new phpmyadmin_Types_MySQL();
        }
        ...
        /**
         * the phpmyadmin_List_Database class
         */
        include_once './libraries/phpmyadmin.php';
        $phpmyadmin = new phpmyadmin;
        $phpmyadmin->userlink = $userlink;
        $phpmyadmin->controllink = $controllink;
    ...
    if (isset($_REQUEST['profiling'])
        && phpmyadmin_Util::profilingSupported()
    ) {
     ...
     */
    if (! defined('phpmyadmin_BYPASS_GET_INSTANCE')) {
        $response = phpmyadmin_Response::getInstance();
    }
    ...
    if (isset($response) && $response->isAjax() && $token_mismatch) {
        ...
        $response->addJSON(
            'message',
            phpmyadmin_Message::error(__('Error: Token mismatch'))
        );
        exit;
    }
} // end if !defined('phpmyadmin_MINIMUM_COMMON')

// load user preferences
$GLOBALS['phpmyadmin_Config']->loadUserPreferences();

// remove sensitive values from session
$GLOBALS['phpmyadmin_Config']->set('blowfish_secret', '');
$GLOBALS['phpmyadmin_Config']->set('Servers', '');
$GLOBALS['phpmyadmin_Config']->set('default_server', '');

/* Tell tracker that it can actually work */
phpmyadmin_Tracker::enable();
...
if (isset($_REQUEST['GLOBALS']) || isset($_FILES['GLOBALS'])) {
    phpmyadmin_fatalError(__("GLOBALS overwrite attempt"));
}
...
if (count($_REQUEST) > 1000) {
    phpmyadmin_fatalError(__('possible exploit'));
}
...
foreach ($GLOBALS as $key => $dummy) {
    if (is_numeric($key)) {
        phpmyadmin_fatalError(__('numeric key detected'));
    }
}
unset($dummy);
